...
public:
	Flight* next;
	Flight(string city = "", int day = 0, int month = 0, int year = 0, int dHour = 0, int dMin = 0, int aHour = 0, int aMin = 0, int cost = 0, string ss1 = "")
	{
		cityname = new string(city);
		airline = new string(ss1);
		fare = new int(cost);
		date = new Date(day, month, year);
		deptime = new Time(dHour, dMin);
		arrtime = new Time(aHour, aMin);
		next = NULL;	
	}
	Date getdate() {
		return *date;
